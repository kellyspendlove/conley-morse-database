...
// User interface: method to be provided by user
  ...
  RectGeo operator () ( const RectGeo & rectangle ) const {    
    ...
    interval w = getRectangleComponent ( rectangle, 3 );
    // Evaluate map
    interval x1 = x / ( x + (1.0-x)*exp(a*( (y+z+w) - b )) );
    interval y1 = y / ( y + (1.0-y)*exp(a*( (x+z+w) - b )) );
    interval z1 = z / ( z + (1.0-z)*exp(a*( (x+y+w) - b )) );
    interval w1 = w / ( w + (1.0-w)*exp(a*( (x+y+z) - b )) );
    
    // Return result
    return makeRectangle ( x1, y1, z1, w1 );
  } 
...
private:
  ...
  RectGeo makeRectangle ( interval const& x1, interval const& y1, interval const& z1, interval const& w1) const {
    ...
    return_value . lower_bounds [ 3 ] = w1 . lower ();
    return_value . upper_bounds [ 3 ] = w1 . upper ();
    return return_value;
  }
};
