...
// User interface: method to be provided by user
  ...
  RectGeo operator () ( const RectGeo & rectangle ) const {    
    ...
    */
    //Handle via map to origin (exclude on diagonal)
    /*
    if (x0.lower() + x1.lower() >= 1.0) {
	x0 = interval(0.0,0.0);
	x1 = interval(0.0,0.0);
    ...
    */
    //Handle via map to origin (include on diagonal)
    if ( (x0.lower() + x1.upper() >= 1.0) || (y0.lower()+y1.upper() >= 1.0  )){
	x0 = interval(0.0,0.0);
	x1 = interval(0.0,0.0);
	y0 = interval(0.0,0.0);
	y1 = interval(0.0,0.0);
    }
    //Handle via map outside of simplex
    /*if ( x0 . lower() + x1 . lower() >= 1.0 ){
	return makeRectangle ( interval(1.5,1.5), interval(1.5,1.5), y0, y1 );
    }
    if ( y0 . lower() + y1 . lower() >= 1.0 ){
	return makeRectangle ( x0, x1, interval(1.5,1.5), interval(1.5,1.5) );
    }*/
